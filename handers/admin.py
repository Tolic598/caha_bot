...
from keyboards import kbadmin, kbpt, kbadmin_chat,otmenaadmin
...
async def greetings_chat_off(call: CallbackQuery):
    id = call.from_user.id
    with connection.cursor() as cursors:
        cursors.execute(f"SELECT * FROM chat WHERE id={id}")
        for row in cursors:
            greetings_set = row['greetings_set']
            await call.message.answer(f"Приветствие: Отключены",reply_markup=kbadmin_chat)
            cursors.execute("UPDATE chat SET greetings_set=%s WHERE id=%s",(0,id))
    connection.commit()
'''#''''''''''''''''''''''''''''''''''''''''Включить приветствие''''''''''''''''''''''''''''''''''''''''''''#'''
async def greetings_chat_on(call: CallbackQuery):
    id = call.from_user.id
    with connection.cursor() as cursors:
        cursors.execute(f"SELECT * FROM chat WHERE id={id}")
        for row in cursors:
            greetings_set = row['greetings_set']
            await call.message.answer(f"Приветствие: ✅Включены✅",reply_markup=kbadmin_chat)
            cursors.execute("UPDATE chat SET greetings_set=%s WHERE id=%s",(1,id))
    connection.commit()
...
'''#''''''''''''''''''''''''''''''''''''''''рассылка  отмена''''''''''''''''''''''''''''''''''''''''''''#'''
async def otmena_admin(message: types.Message, state: FSMContext):
    current_state = await state.get_state()
    if current_state is None:
        return
    await state.finish()
    await message.reply("✅Успешная отмена ✅",reply_markup=kbadmin)
