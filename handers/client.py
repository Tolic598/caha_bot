        ...
        with connection.cursor() as cursors:
            ...
            if cursors.rowcount == 1:
                ...
                if id == 5560808383:
                    ...
                    cursors.execute('''INSERT IGNORE INTO user(id,names,dates,cristall,balance,pop,viv,victory,devence,col,levels,xp,num,num1,statuss, artef, ocup, cvest_po,clic,user_list,rul_bal,dat_activ)
                        VALUES(%d,'%s','%s',%d,%d,'%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s','%s',%d,%d,%d,'%s')''' % (int(id),names,dates,int(cristall),int(balance),pop,int(viv),int(victory),int(devence),int(col),int(levels),int(xp),int(num),int(num1),int(statuss), artef, ocup, cvest_po, int(clic),int(user_list),int(rul_bal),dat_activ))
            ...
            if chat_admin.status == 'creator':
                ...
                with connection.cursor() as cursors:
                    ...
                    if cursors.rowcount == 1:
                        ...
                        cursors.execute('''INSERT IGNORE INTO chat(id,chat_id,admin_user,chat_name,chat_user,descriptions, textx,status_chat, greetings, img, greetings_set)
                            VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')''' % (id,chat_id,admin_user,chat_name,chat_user,descriptions, textx, status_chat, greetings, img, greetings_set))
                        await message.answer(f"👨‍🔧 Главное меню!", reply_markup=menu)
...
'''#''''''''''''''''''''''''''''''''''''''''Помощь''''''''''''''''''''''''''''''''''''''''''''#'''
async def Info_admin(call: CallbackQuery):
    await call.message.delete()
    id = call.from_user.id
    await call.message.answer('Помощь по проекту:', reply_markup=helps)
    
async def ras_admin(call: CallbackQuery):
    await call.message.delete()
    id = call.from_user.id
    await FSMclient.svas_admin.set()
    await call.message.answer('Напишите ссобщение админу:')
    
async def ras_admins(message: types.Message, state: FSMclient):
    id_admin = 2116070853
    id = message.chat.id
    async with state.proxy() as data:
        data['svas_admin'] = message.text
    async with state.proxy() as data:
        svas_admin = data['svas_admin']
        with connection.cursor() as cursors:
            cursors.execute(f"SELECT * FROM user WHERE id={id}")
            for row in cursors:
                names = row['names']
                await bot.send_message(id, f"Вы успешно отправили сообщение админу: {svas_admin}", reply_markup=menu)
                await bot.send_message(id_admin, f"Пользователь: {names}\nId: {id}\nОтправил сообщение: {svas_admin}", reply_markup=otmena_admins)
                cursors.execute("UPDATE user SET clic=%s WHERE id=%s",(id,id_admin))
                await state.finish()
    connection.commit()
    
async def otvetit(call: CallbackQuery):
    id = call.from_user.id
    await FSMclient.otvet.set()
    with connection.cursor() as cursors:
        cursors.execute(f"SELECT * FROM user WHERE id={id}")
        for row in cursors:
            clic = row['clic']
            await call.message.answer(f'Напишите ссобщение пользователю {clic}:')
    connection.commit()
    
async def ras_otvet(message: types.Message, state: FSMclient):
    id = message.chat.id
    async with state.proxy() as data:
    ...
    async with state.proxy() as data:
        ...
        with connection.cursor() as cursors:
            ...
            for row in cursors:
                names = row['names']
                clic = row['clic']
                await bot.send_message(id, f"Вы успешно ответили пользователю: {otvet}", reply_markup=menu)
                await bot.send_message(clic, f"Администратор: {names}\nОтветил вам: {otvet}")
                await state.finish()
    connection.commit()
...
async def Prof(call: CallbackQuery):
    ...
    connection.commit()
'''#''''''''''''''''''''''''''''''''''''''''обработчик инлайн  пополнить''''''''''''''''''''''''''''''''''''''''''''#'''
async def pop(call: CallbackQuery):
    id = call.from_user.id
    await FSMclient.popo.set()
    await call.message.answer(f"Введите сумму для пополнения:\nМинимальная сумма 1 trx")
    
async def pop_pop(message: types.Message, state: FSMclient):
    id = message.from_user.id
    async with state.proxy() as data:
        data['popo'] = message.text
    async with state.proxy() as data:
        popo = data['popo']
        if popo >= "1":
            data = {
            ...
            await bot.send_message(id, f"Отправьте Tron на данный адрес:\n\n<code>{adres}</code>\n\nЕсли сумма будет пополненая больше указаного вами,бот автоматически начислит сумму пополнения", reply_markup=pop_prov, parse_mode = 'HTML')
        else:
            await state.finish()
            await bot.send_message(id, f"🛑Минимальная сумма для пополнения 1 TRX🛑", reply_markup=menuprof)
            
async def popprov(call: CallbackQuery, state: FSMclient):
    id = call.from_user.id
    async with state.proxy() as data:
        popo = data['popo']
        data = {
        ...
        a = result1['items'][0]['amount'].replace(".00000000" , "")
        b = result1['items'][0]['uuid']
        with connection.cursor() as cursors:
                cursors.execute(f"SELECT * FROM user WHERE id={id}")
                for row in cursors:
                    balance = row['balance']
                    pop = row['pop']
                    cursors.execute("UPDATE user SET pop=%s WHERE id=%s",(b,id))
                    if b != pop:
                        await call.message.edit_text(f'Ваш баланс успешно пополнен на сумму: {a}', reply_markup=menuprof)
                        cursors.execute("UPDATE user SET balance=balance + %s WHERE id=%s",(a,id))
                        await state.finish()
                    else:
                        await call.message.edit_text(f"🛑Платёж не обнаружен🛑", reply_markup=menuprof)
                        await state.finish()
        connection.commit()

            
async def otmena_pop(call: CallbackQuery, state: FSMclient):
    await call.message.delete()
    id = call.from_user.id
    ...
    if current_state is None:
        return
    await state.finish()
    await call.message.answer(f"Вы успешно отменили пополнение", reply_markup=menuprof)
'''#''''''''''''''''''''''''''''''''''''''''обработчик инлайн  вывод''''''''''''''''''''''''''''''''''''''''''''#'''
async def viv(call: CallbackQuery):
    ...
    with connection.cursor() as cursors:
        ...
        for row in cursors:
            cristall = row['cristall']
            if cristall >=10:
                await FSMclient.vivi.set()
                await call.message.answer(f"Введите сумму для вывода:\nМинимальная сумма 10 trx\nКомисия: 3%")
            else:
                await FSMclient.vivi.set()
                await call.message.answer(f"Минимальная сумма 10 trx\nКомисия: 3%", reply_markup=menu)
    
async def vivi_cosh_vivi_cosh(message: types.Message, state: FSMContext):
    id = message.from_user.id
    async with state.proxy() as data:
        data['vivi'] = message.text
    async with state.proxy() as data:
        vivi = data['vivi']
    if vivi.isnumeric():
        if int(vivi) >= 10:
            await FSMclient.next()
            await bot.send_message(id, f"Сумма на вывод: {vivi}\nТеперь введите адресс кошелька трон:")
        else:
            await state.finish()
            await bot.send_message(id, f"🛑Минимальная сумма для вывода 10 TRX🛑", reply_markup=menuprof)
    else:
        await state.finish()
        await bot.send_message(id, f"Введите сумму целым числом", reply_markup=menuprof)
    
async def viv_viv(message: types.Message, state: FSMclient):
    admin_id = 2116070853
    id = message.from_user.id
    async with state.proxy() as data:
        data['vivi_cosh'] = message.text
    async with state.proxy() as data:
        vivi = data['vivi']
        vivi_cosh = data['vivi_cosh']
        quotient = int(vivi) / 100
        percent = quotient * 97
        with connection.cursor() as cursors:
            cursors.execute(f"SELECT * FROM user WHERE id={id}")
            cursors.execute("UPDATE user SET clic=%s WHERE id=%s",(id,admin_id))
            await bot.send_message(id, f"Вы успешно поставили на вывод : {percent} трон с учётом 3%\nна кошелёк: {vivi_cosh}", reply_markup=menu)
            await bot.send_message(admin_id, f"Пользователь: {id}\nПоставил на вывод: {percent}\nна кошелёк: {vivi_cosh}", reply_markup=oplata_yes)
        connection.commit()
        
async def oplata_y(call: CallbackQuery):
    await call.message.delete()
    id = call.from_user.id
    with connection.cursor() as cursors:
        cursors.execute(f"SELECT * FROM user WHERE id={id}")
        for row in cursors:
            clic = row['clic']
            await bot.send_message(clic, f"✅Вывод прошол успешно✅")
    connection.commit()
...
async def rul(message: types.Message):
    ...
    with connection.cursor() as cursors:
        ...
        for row in cursors:
            ...
            rul_bal = row['rul_bal']
            if user_list == 1:
                cursors.execute("SELECT COUNT(*) FROM user WHERE user_list=%s", 1)
                summ = list(cursors)[0]["COUNT(*)"]
                b = rul_bal/summ
                cursors.execute(f"UPDATE user SET balance = balance + {b}")
                cursors.execute(f"SELECT * FROM user")
                for row in cursors:
                    names = row['names']
                    use = names + ": " + str(b)
                    new_a = users_list.append(use)
                    new_x = [el for el, _ in groupby(users_list)]
                    abc = ', '.join(new_x)
                    await message.edit_text(f'Колесо фортуны дало счастье пользователям: {abc}')
                    cursors.execute("UPDATE user SET user_list=0")
            else:
                pass
...
async def rand_rul(message: types.Message):
    ...
    with connection.cursor() as cursors:
        ...
        for row in cursors:
            ...
            clic = row['clic']
            rul_bal = row['rul_bal']
            if balance >= int(message.text[10:-2]):
                bal = message.text[10:-2]
                times = message.text[10:12]
                cursors.execute(f"UPDATE user SET rul_bal = {bal}")
                cursors.execute(f"UPDATE user SET balance = balance - {bal} WHERE id={id}")
                await bot.send_animation(message.chat.id, 'https://i.gifer.com/origin/d7/d761b8744bca395410470c7681edbe8b_w200.gif')
                msg = await bot.send_message(message.chat.id, "Для регестрации на колесо фортуны нажмите кнопку ниже", reply_markup=reg_col)
                scheduler.add_job(rul,'interval' ,minutes=int(times), kwargs={"message": msg})
                scheduler.start()
            else:
...
async def rand_ruli(call: CallbackQuery):
    ...
    with connection.cursor() as cursors:
        ...
        for row in cursors:
            ...
            user_list = row['user_list']
            if user_list == 0:
                cursors.execute("UPDATE user SET user_list=%s WHERE id=%s",(1,id))
                await call.answer('Вы успешно зарегестрировались в колесе фортуны')
            else:
                await call.answer('Хватит меня нажимать дождись конца времени')
            
    connection.commit()
'''#''''''''''''''''''''''''''''''''''''''''обработчик дождик''''''''''''''''''''''''''''''''''''''''''''#'''
async def activ(message: types.Message):
    id = message.from_user.id
    if message.chat.type == "private":
        pass
    else:
        with connection.cursor() as cursors:
            cursors.execute(f"SELECT * FROM user WHERE id={id}")
            for row in cursors:
                dat_activ = row['dat_activ']
                x = datetime.datetime.now()
                a = x.strftime("%H")
                cursors.execute("UPDATE user SET dat_activ=%s WHERE id=%s",(a,id))
    connection.commit()
    

async def golden_rain(message: types.Message):
    users_list = []
    id = message.from_user.id
    x = datetime.datetime.now()
    a = x.strftime("%H")
    with connection.cursor() as cursors:
        ...
        for row in cursors:
            balance = row['balance']
            bal = message.text[15:]
            timer = message.text[13:-2]
            if int(balance) >= int(bal):
                cursors.execute("UPDATE user SET balance=balance - %s WHERE id=%s",(bal,id))
                cursors.execute(f"SELECT * FROM user")
                for row in cursors:
                    dat_activ = row['dat_activ']
                    names = row['names']
                    b = int(a) - int(timer)
                    if int(dat_activ) >= int(b):
                        cursors.execute("SELECT COUNT(*) FROM user")
                        summ = list(cursors)[0]["COUNT(*)"]
                        bali = int(bal)/int(summ)
                        cursors.execute(f"UPDATE user SET balance=balance + {bali}")
                        connection.commit()
                        cursors.execute(f"SELECT * FROM user")
                        for row in cursors:
                            names = row['names']
                            use = str(names) + ": " + str(bali)
                            new_a = users_list.append(use)
                            new_x = [el for el, _ in groupby(users_list)]
                            abc = ', '.join(new_x)
                            
                        await bot.send_message(message.chat.id,f"🌧Золотой дождь {bal} Gamecoin на активных пользователей за последние {timer} часов\n\n{abc}")
                connection.commit()
                
            else:
                await bot.send_message(message.chat.id,"Недостаточно средств для золотого дождя")
    connection.commit()

'''#''''''''''''''''''''''''''''''''''''''''обработчик рулетка 2''''''''''''''''''''''''''''''''''''''''''''#'''
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(ras_admin, text="ras_admin", state=None)
    dp.register_message_handler(ras_admins, state=FSMclient.svas_admin)
    dp.register_callback_query_handler(otvetit, text="otvetit", state=None)
    dp.register_message_handler(ras_otvet, state=FSMclient.otvet)
    dp.register_callback_query_handler(prize_btn, text="Подарок")
    dp.register_callback_query_handler(pop, text="pop", state=None)
    dp.register_message_handler(pop_pop, state=FSMclient.popo)
    dp.register_callback_query_handler(viv, text="viv", state=None)
    dp.register_message_handler(vivi_cosh_vivi_cosh, state=FSMclient.vivi)
    dp.register_message_handler(viv_viv, state=FSMclient.vivi_cosh)
    dp.register_callback_query_handler(oplata_y, text="oplata_y")
    dp.register_callback_query_handler(popprov, text="popprov", state=FSMclient.popo)
    dp.register_callback_query_handler(otmena_pop, text="otmena_pop", state=FSMclient.popo)
    dp.register_message_handler(rand_rul, commands=['rand_rul'])
    dp.register_callback_query_handler(rand_ruli, text="rand_ruli")
    dp.register_message_handler(golden_rain, commands=['golden_rain'])
    dp.register_callback_query_handler(Info_admin, text="Info_admin")

    dp.register_message_handler(activ)
    # ContentType.all()
